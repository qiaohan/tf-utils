...
class TypeNet(CNN):
    def build_loss(self):
        ...
        conv_feats_reshape = tf.reshape(self.conv_feats,[self.batch_size,feadim])
        #self.logits = fully_connected(conv_feats_reshape,3,"cls_logits",init_w='normal', stddev=0.01)
        ...
        self.logits = fully_connected(self.feat_embbed,3,"cls_logits",init_w='xavier', stddev=0.01)
    ...
    def test(self, sess):
        ...
        for idx in tqdm(list(range(test_dataset.num_batches)), desc='test'):
            ...
            img_files, clables, ctypes = batch
            [acc] = sess.run([self.acc], feed_dict={self.img_files:img_files, self.is_train:False, self.clsgt:ctypes}) 
    ...
    def train(self, sess):
        ...
        for epoch_no in tqdm(list(range(params.num_epochs)), desc='epoch'): 
            for idx in tqdm(list(range(train_dataset.num_batches)), desc='batch'): 
                ...
                _, loss, global_step = sess.run([self.opt_op, self.loss, self.global_step], feed_dict={self.img_files:img_files, self.is_train:True, self.clsgt:ctypes}) 
                print(" loss=%f " %(loss)) 

                #if (idx+1) % params.test_period == 0:
                #	self.test(se
                if (idx+1) % params.save_period == 0:
                    self.save(sess, npyfile = "ckpt_npy/typenet.npy")
            train_dataset.reset()
        print("Model trained.")
